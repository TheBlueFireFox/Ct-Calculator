...
function setInput(from) {
        ...
        document.getElementById('input' + from + 'sDecimal').value = value.get_signed
        document.getElementById('input' + from + 'uDecimal').value = value.get_unsigned
...
function calculateResult() {
        ...
        let rawLeft = document.getElementById('inputLeftsDecimal').value
        let rawRight = document.getElementById('inputRightsDecimal').value
        ...
        switch (operation) {
                ...
                case "xor":
                        ...
                        break
                case "nand":
                        res = wasm.nand(left, right, chosenBits)
                        break
                default:
...
function resetField(loc) {
        for (let [type] of inputFields) {
                document.getElementById('input' + loc + type).value = ''
        }
        document.getElementById('input' + loc + "2C").value = ''
        resetResults()
}
...
function main() {
        ...
        // get currently choses
        chosenBits = parseInt(document.querySelector('input[name = "bitAmount"]:checked').value)
        ...
        for (let [type, base] of inputFields) {
                for (let loc of ['Left', 'Right']) {
                        document.getElementById('input' + loc + type).addEventListener('keyup', (e) => {
